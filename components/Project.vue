<template>
   <nuxt-link :to="`/works/${data.code}`" :class="{ 'relative': column, 'absolute': !column, 'grayscale': !active }" class="__EIGHTEEN-PROJECT z-50 lg:py-42 py-24 w-full h-auto transition-all">
      ...
      <div class="__EIGHTEEN-PROJECT-TITLE opacity-0 absolute transition-all top-5 left-4 text-black">
         <div class="project-line border-t border-x-0 border-b-0 border-solid border-black h-px w-[48px] pb-4" />
         ...
         <button class="border-black px-4 py-2 border-solid border mt-4 uppercase text-xs hover:bg-black hover:text-white transition text-black">See more</button>
      </div>
   </nuxt-link>
</template>
