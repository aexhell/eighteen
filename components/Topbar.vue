<template>
   <header role="banner" class="w-full relative z-20" id="__EIGHTEEN-TOPBAR">
      <nav role="navigation" class="md:py-12 md:px-24 px-8 md:border-none border-b border-b-solid border-black/10 dark:border-white/10 py-8 flex items-center dark:text-white text-black justify-between">
         <nuxt-link to="/" alt="Aexhell logo" aria-label="Aexhell logo" class="font-bold no-underline uppercase">
            <svg class="w-10 h-10 text-black dark:text-white" width="180" height="208" viewBox="0 0 180 208" fill="none" xmlns="http://www.w3.org/2000/svg">
         ...
         <div class="list-none flex justify-end items-center gap-4 p-0 m-0">
            ...
            <button :alt="`${darkMode ? 'Dark' : 'Light'} Theme`" :aria-label="`${darkMode ? 'Dark' : 'Light'} Theme`"  class="bg-transparent flex outline-none p-0 dark:text-white rounded-full cursor-pointer" @click="darkModeChange">
               <svg v-if="!darkMode" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="md:w-6 md:h-6 w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" />
               </svg>
               <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="md:w-6 md:h-6 w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
               </svg>
            </button>
            <div class="rounded-lg hidden md:block bg-white/20 text-black dark:text-white dark:bg-white/10 md:px-4 px-2 py-1 border border-solid dark:border-white/20 backdrop-blur-[2px] relative z-10 uppercase text-xs border-black/20">
               {{ date }}
            </div>
         </div>
      </nav>
